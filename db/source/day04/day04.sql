/*
    참고 ]
        HAVING 절
        ==> 그룹화한 결과를 필터링을 할 때 필터링 조건을 기술하는 절
...
/*
    ...
    ==> Relation Database Management System 에서는 데이터의 중복을 피하기 위해서
        테이블을 분리함으로써 테이블들 간의 관계가 형성이 되었다.
        
        그런 분리된 테이블에서 데이터를 추출해낼 때 사용하는 문법이 JOIN이다.
        
        참고 ]
            오라클 역시 ER 형태의 데이터베이스이다.
            ER 
                엔티티(테이블)끼리의 관계를 이야기하고 이런 관계들로 테이블을 관리하는 데이터베이스를 관계형 데이터베이스라고 한다.
                
    참고 ]
        관계형 데이터베이스에서는 여러개의 테이블에서 동시에 검색하는 기능은 이미 가지고 있다.
        ==> 이때 여러개의 테이블에서 데이터를 동시에 검색하면 
            Cartesian Product(CROSS JOIN)가 만들어지는데, 이 결과에는 정확하지 않은 데이터도 포함되어있다.
            따라서 정확한 데이터만 필터링해서 꺼내와야 하는데 이때 이 필터링하는 작업을 JOIN이라고 한다.
    ...
    종류 ]
        ...
        Outer Join
        ==> Catesian Product에 포함되지 않는 데이터를 가져오는 조인
...
CREATE TABLE ecolor (
    ...
    code VARCHAR2(7)    -- 컬러 코드값
        ...
        CONSTRAINT ECLR_CODE_UN NOT NULL,
    name VARCHAR2(20)
...
-- 영문 컬러 테이블 조회
SELECT * FROM ecolor;
COMMIT; -- 메모리의 작업 영역에서 작업한 내용을 데이터베이스에 적용시키는 명령
...
CREATE TABLE kcolor (
    ...
    code VARCHAR2(7)
        ...
        CONSTRAINT KCLR_CODE_UN NOT NULL,
    name VARCHAR2(20)
...
FROM
    emp e, emp s
WHERE
    e.mgr = s.empno(+)  -- NULL로 표현되어야 할 데이터를 갖고 있는 테이블에 (+)
;
...
/*
    ...
    ==> 질의 명령은 데이터베이스(DBMS)에 따라서 약간씩 그 문법이 달라진다.
        ...
        ANSI 형식이란
        미국 국립 표준 협회(ANSI)에서 공통의 질의 명령을 만들고자 해서 통일된 방식의 명령을 만들어 놓은것
    ...
    3. Outer Join
        ==> Certesion Product에 없는 결과를 조회하는 조인 명령
        
        형식 ]
            SELECT
                필드이름, ...
            FROM
                테이블1 LEFT 또는 RIGTH 또는 FULL OUTER JOIN 테이블2
                ==> 이때 방향은 데이터가 있는 테이블 방향을 기술하면 된다.(NULL이 아닌 곳)
            ON
                조인 조건
            ;
...
-- 81년 입사한 사원들의 사원이름, 직급, 입사년도, 부서이름을 주회하세요.
SELECT
    ename 사원이름, job 직급, TO_CHAR(hiredate, 'YY"년"') 입사년도, dname 부서이름
FROM
    emp e JOIN dept d
ON
    e.deptno = d.deptno
...
/*
    NATURAL JOIN
    ==> 자동 조인
        반드시 조인 조건식에 사용하는 필드의 이름이 동일하고 반드시 "동일한 필드가 한개"일 때 사용할 수 있는 조인
        자동으로 중복되는 필드를 사용해서 조인을 하기 때문에 조인조건을 기술하지 않는다.
...
/*
    부질의(SUQ QUERY, 서브질의)
    ==> 질의 명령 안에 다시 질의 명령을 포함하는 경우
        포함되는 그 질의 명령을 서브질의 또는 서브쿼리라고 부른다.
        
        예 ]
            이름이 SMITH인 사원과 같은 부서에 있는 사원들의 정보를 조회하세요.
            ==> 이 경우 SMITH의 부서번호를 알아내기 위해서 먼저 질의 명령이 실행되어야 한다.
            
            ==> 꺼내온 부서번호를 이용해서 정보를 조회
            
            이때 위의 질의 명령을 아래 메인 질의 명령에서 조회해서 사용해야 한다.
    ...
    서브질의의 위치에 따른 결과
        ...
        ==> FROM 절에는 테이블이 나열되어야 한다.
            ...
            이때 FROM 절 안에 들어가는 서브질의를 특별히 마치 테이블과 같다해서
                INLINE TABLE    이라고 부른다.
                
            이때 사용은 질의 명령을 보낼 때 사용한 별칭을 사용해서 데이터를 꺼내야 한다.
...
-- 10번 부서 사원들과 같은 직급의 직급 급여 평균을 구하세요.
SELECT
    job 직급, FLOOR(AVG(sal)) 직급급여평균
FROM
    emp
WHERE
    job IN (
            ...
            FROM
                emp
            WHERE
                deptno = 10     -- 질의 명령의 결과는 다중값으로 발생한다. MANAGER, PRESIDENT, CLERK
        )
GROUP BY
    job
;
...
-- ANY
-- 각 부서의 평균 급여보다 한 부서라도 급여가 높은 사원들의 사원이름, 사원급여, 부서번호를 조회하세요.
SELECT
...
-- 모든 각 부서의 급여 평균보다 높은 급여를 받는 사원들의 사원이름, 사원급여, 부서번호를 조회하세요.
SELECT
    ename 사원이름, sal 사원급여, deptno 부서번호
FROM
    emp
...
SELECT
    ename 사원이름, deptno 부서번호, cnt "부서원 수", avg 부서평균급여, sum 부서급여합계
FROM
    emp,
    (
        SELECT
...
/*
    조인 명령을 일반 명령보다 먼저 쓰는게 좋다!
    테이블을 먼저 조인시켜주면 빠르다!
    둘이 겹치는 컬럼이 하나로 조인 되기 때문에 조인 전에는 두개에서 봐야한다면 조인 후에는 하나의 컬럼만 확인하면 되기 때문에
*/