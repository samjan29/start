...
/*
    댓글형 게시판의 경우
    목록을 꺼내올 때 상위글 바로 다음에 그 글의 댓글이 조회가 되어야 한다.
...
/*
    ...
    따라서 CRUD를 이야기하면 DML 명령을 사용할 수 있는지를 이야기하는 것이 된다.
    
    이제 우리가 배운 명령들은 모두 CRUD 중에서 R에 해당하는
    SELECT 질의 명령만 공부했는데
    ...
    1. INSERT
        ...
        필드에 설정된 디폴트 값으로 입력되게 하려면 필드를 나열할 때 생략하면 된다.
*/
...
INSERT INTO
    emp1
VALUES(
    1001, '감자', '사장', null, sysdate, 1000, 50, 99
);
...
-- emp1 테이블에 당근 사원을 추가하세요.
INSERT INTO
    emp1(ename, empno)
VALUES(
--    1003, '당근'    순서가 맞지 않아서 입력오류
    '당근', 1003
);
...
/*
    **********
    참고 ]
        DML 명령은 메모리 상에 확보한 세션 공간에서만 작업이 이루어진다.
        따라서 데이터베이스에 적용은 안되어있는 상태이다.
        작업내용을 데이터베이스에 적용되기를 원하면 반드시 commit 명령을 실행해야 한다.
*/
...
/*
    2. UPDATE
        ...
        ***
        참고 ]
            WHERE 조건절이 없는 경우는 모든 데이터가 같은 내용으로 수정된다.
*/
...
/*
    ...
    이때 여러개의 필드를 동시에 질의 명령의 결과로 대체하고자 한다면
    ...
    참고 ]
        만약 수정을 원하지 않는 데이터를 반드시 써야한다면 현재 데이터를 그대로 사용해도 된다.
*/

-- 당근의 직급과 급여를 EMP 테이블의 SMITH 사원의 데이터를 복사해서 수정하세요.
UPDATE
    emp2
SET
    (job, sal) = (
...
/*
    DELETE
    ==> 현재 데이터 중에서 필요하지 않는 데이터를 삭제하는 명령 한행을 통째로 지운다는 의미
...
/*
    참고 ]
        테이블 설계
            ...
            따라서 테이블을 만들 때는 여러가지 규칙을 정해놓고(정규화 규칙)
            ...
            데이터베이스 설계 과정
                ...
                2. 논리적 설계
                    ==> 개념적 설계의 결과를 물리적 저장장치에 저장할 수 있도록
                        특정 데이터베이스가 지원하는 논리적인 자료구조로 변화시키는 과정
