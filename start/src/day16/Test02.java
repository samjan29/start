...
// HashTable에 친구의 정보를 기억시켜보자.
	...
	public Test02() {
		...
		// 데이터 추가
		table.put("이름", "제니");
		// 이처럼 Map계열은 데이터를 입력할 때 그 데이터를 구분할 키값과 데이터를 한쌍으로 만들어서 입력해야만 한다.
		table.put("나이", 27);
		...
		/*
		 	Map은 같은 키값으로 데이터가 입력되면 먼저 데이터를 삭제한 후 새로운 데이터로 교체한다.
		...
		while(it3.hasNext()) {
			Map.Entry en1 = (Map.Entry) it3.next();	//Map.Entry는 VO 클래스
