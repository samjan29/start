...
public class Test07 {
	...
	public Test07() {
		/*
		 	참고 ]
		 		키보드 역시 입력 장치로 외부 장치다.
		 		자주 사용하는 외부 장치이므로 이미 스트림을 준비한 변수가 존재한다.
		 		
		 			System.in
		 */
		System.out.print("입력해주세요! : ");
		int ch = 0;		// 함수 블록 안에서 실행될 경우 초기화가 되지 않아 에러가 발생함
		try {
			/*
			// 1. 한글자만 읽어보자.
			ch = System.in.read();
		 ...
		 */
			...
			byte[] buff = new byte[256];
			// 배열에 기본값을 'A'로 채워보자.
			Arrays.fill(buff, (byte)'A');	// ==> 배열의 모든 방에 'A'로 채워넣는다.	char는 2바이트기 때문에 byte로 강제 형 변환
