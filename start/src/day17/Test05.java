...
public class Test05 {
	...
	public Test05() {
		...
		for(File f : infoList) {
			...
			long len = f.length();			// 파일 시이즈 추출해주는 함수
			String pold = f.getParent();	// 상위 폴더 이름 꺼내주는 함수
		...
		*/
		...
		String[] fList = dir.list(new FilenameFilter() {
			...
			public boolean accept(File dir, String name) {
				...
				if(name.endsWith("png")) {	// 파일 이름이 jpg로 끝나는가?
					// 이 경우는 결과 목록에 이 파일을 포함시킨다.
					bool = true;
				}	// 변수 bool은 초깃값으로 이미 false로 세팅되어 있으므로 생략해도 괜찮음
